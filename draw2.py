...
#    )    
    ...
    def auto_step (span):
        """根据经纬度跨度自动选择合适的间隔"""
        if span > 180: # 跨度大于 180° 时使用 60° 间隔
            return 60
        elif span > 120: # 跨度大于 180° 时使用 60° 间隔
            return 40
        elif span > 90: # 跨度大于 180° 时使用 60° 间隔
            return 30
        elif span > 60: # 跨度大于 60° 时使用 20° 间隔
            return 20
    ...
    for lon in lon_lines:
        if min_lon + lon_step/10 <= lon <= max_lon - lon_step/10:
            ...
            lon_label = f"{abs(int(lon))}°{lon_dir}"        
            ax.text(
...
if __name__ == "__main__":
    ...
    y1,x1 = transformer.transform(22.472, 59.831)  # 左上角 必须先纬度后经度
    y2,x2 = transformer.transform(8.277, 77.322)   # 右下角
    
    print(x1, y1) 
    bbox = [min(x1,x2), min(y1,y2), max(x1,x2), max(y1,y2)]
    add_graticule_to_web_mercator("map.png", "output2.png", bbox)

def generate_lon_lat_charts(base64_list, bbox):
    import base64
    import tempfile
    import os
    ...
    # 转换坐标到Web墨卡托
    y1, x1 = transformer.transform(bbox[1], bbox[0])  # 左上纬度,经度
    y2, x2 = transformer.transform(bbox[3], bbox[2])   # 右下纬度,经度
    mercator_bbox = [min(x1, x2), min(y1, y2), max(x1, x2), max(y1, y2)]
    ...
    for img_base64 in base64_list:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as tmp_input:
            tmp_input.write(img_data)
            
        tmp_output = tempfile.NamedTemporaryFile(delete=False, suffix='.png')
        tmp_output.close()
        
        # 调用现有处理逻辑
        add_graticule_to_web_mercator(tmp_input.name, tmp_output.name, mercator_bbox)
