...
def run_fss_ts(rain_scale,expn,cycl,starttime,endtime,length,obs_o,area,half_size_str,area_id):
    ...
    ## clear old files
    clear_path = os.path.join(testdir,f'OBS/{obs_o}_{expn}_{area_id}_AWS/')
    if os.path.exists(clear_path) and os.path.isdir(clear_path):
        if True:
            shutil.rmtree(clear_path)
        #except Exception as e:
            #print(f'error deleting {clear_path}: {e}')
    os.makedirs(clear_path)
    obsroot_ii = os.path.join(testdir, f"OBS/{obs_o}_{expn}_{area_id}_AWS/obs{rain_scale}.")
    FCST_path  = os.path.join(testdir, f"FCST/{rain_scale}_{expn}_{area_id}_AWS")
    ...
    # Process landmask
    if os.path.exists(os.path.join(testdir, f'mask_region_{area_id}_{expn}_AWS.grib')):
        os.remove(os.path.join(testdir, f'mask_region_{area_id}_{expn}_AWS.grib'))
    cdo_path    = config["cdo"]
    input_grib  = os.path.join(testdir, mask_filen)
    output_grib = os.path.join(testdir, f'mask_region_{area_id}_{expn}_AWS.grib')
    cmd0 = f"./cdo remapbil,{grid_file} {input_grib} {output_grib}"
    os.chdir(cdo_path)
    os.system(cmd0)

    maskroot     = os.path.join(testdir, f'mask_region_{area_id}_{expn}_AWS.grib')
    ...
    while current_date <= end_date_obs:
        ...
        output_file = os.path.join(testdir, f"OBS/{obs_o}_{expn}_{area_id}_AWS/obs{rain_scale}.{current_date.strftime('%Y%m%d%H')}.grib")
    ...
    for exp in experiments:
        ...
        outname_final_    = f'{outputdir}/gridrain/fss/data/rain{rain_scale}/{expn}_AWS_region{area_id}_{cycl}'
        ...
        if not os.path.exists(outname_final_):
            os.makedirs(outname_final_)
        outputrootfss_ii = os.path.join(outputrootfss_ii_, "fss")
        outputrootts_ii  = os.path.join(outputrootts_ii_, "tslist")
        outname_final    = os.path.join(outname_final_, "fss")
        ...
        if rain_scale == 24:
            ...
            cmd_2 = f'python /vol8/home/kongjun/VERIFY/met/met_backend/fss24_point.py {cycl} {fcstroot_in} {obsroot_ii} {maskroot} {starttime} {endtime} {lonst} {loned} {latst} {lated} {half_size_str} {FCST_path} {grid_file} {cdo_path} {area_id} {outname_final}'
            os.system(cmd_2)
...
if __name__=="__main__":
    ...
    expn_list = ['CMAGFS','KT1279','ECMWF','NCEP','AUTO']
    obs_n_list  = ['CLDAS']#,'CMPAS','GPM','ERA5','AWS']
    region_n_list = ['100','101','102','103','104','105','106','107']
    rainscale_list = [24]

    if True:
        current_time = (datetime.datetime.now()-datetime.timedelta(days=1)).strftime("%Y%m%d%H")
