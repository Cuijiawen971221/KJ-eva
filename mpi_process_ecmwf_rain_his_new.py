        ...
        for filename in sorted(os.listdir(folder_path)):
            if filename.startswith('W_NAFP_C_ECMF_') and len(filename)==51:
                ...
                try:
                    ...
                    rain_[np.where(rain_<0.0000001)] = 0
...
def run_main_his_ec(model_n,input_directory,outpath1,example_n,mess_path,timenow):
...
if __name__ == "__main__":
    ...
    st_str  = sys.argv[1]     #'2025073100'
    ed_str  = sys.argv[2]     #'2025083100'
    st_t    = datetime.strptime(st_str,"%Y%m%d%H")
    ed_t    = datetime.strptime(ed_str,"%Y%m%d%H")
    current_t = st_t
    while current_t<=ed_t:
        timenow = current_t.strftime("%Y%m%d%H")
        run_main_his_ec(model_n,input_directory,outpath1,example_n,mess_path,timenow)
        current_t += timedelta(days = 1)
