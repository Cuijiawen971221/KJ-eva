...
while(tgT<endTime and tgT< dt.datetime.now()):
    ...
    if not os.path.exists(filePath+f"/{fcst}/{tgT.strftime('%Y%m%d')}/airport_{fcst}_{tgT.strftime('%Y%m%d%H')}.ok"):
        with open(filePath+f"/{fcst}/{tgT.strftime('%Y%m%d')}/airport_{fcst}_{tgT.strftime('%Y%m%d%H')}.retry","w") as f:
            pass
    else:
        if args.force:
            os.remove(filePath+f"/{fcst}/{tgT.strftime('%Y%m%d')}/airport_{fcst}_{tgT.strftime('%Y%m%d%H')}.ok")
            with open(filePath+f"/{fcst}/{tgT.strftime('%Y%m%d')}/airport_{fcst}_{tgT.strftime('%Y%m%d%H')}.retry","w") as f:
                pass
        else:
            print(tgT," is OK")
