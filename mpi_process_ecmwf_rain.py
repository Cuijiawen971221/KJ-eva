...
def check_data(time_str,mess_path):
    ...
    time_str = yyyymmdd
    """
    mess_file_ok   = f'{mess_path}/ECMWF/{time_str[0:8]}/rain24_ECMWF_{time_str}.ok'
    mess_file_retry= f'{mess_path}/ECMWF/{time_str[0:8]}/rain24_ECMWF_{time_str}.retry'
    ## 00/12时刻的检查
    hr_in = time_str[8:10]
    if hr_in=='00':
        file_zh = f'/vol8/home/kongjun/VERIFY/met/met_backend/fcst/ECMWF/rain/rain24/fcst{time_str}240.grb'
    else:
        file_zh = f'/vol8/home/kongjun/VERIFY/met/met_backend/fcst/ECMWF/rain/rain24/fcst{time_str}228.grb'

    if os.path.exists(file_zh):
        if os.path.exists(mess_file_retry):
            os.remove(mess_file_retry)
        if not os.path.exists(mess_file_ok):
            with open(mess_file_ok,"w") as f:
                pass
        indx = True
    else:
        if not os.path.exists(mess_file_retry):
            with open(mess_file_retry,"w") as f:
                pass
        if os.path.exists(mess_file_ok):
            os.remove(mess_file_ok)
        indx = False
...
def save_result(precip_accum,outpath,folder_name,example_n,mess_path_i):
    ...
    if indx_save:                     
        successmess = f'{mess_path_i}/ECMWF/{folder_name[0:8]}/rain24_ECMWF_{folder_name}.ok'
        with open(successmess,"w") as f:
            pass
        try:
            retrynextmess = f'{mess_path_i}/ECMWF/{folder_name[0:8]}/rain24_ECMWF_{folder_name}.retry'
...
if __name__ == "__main__":
    ...
    oknextmess    = f'{mess_path}/ECMWF/{start_time[0:8]}/rain24_ECMWF_{start_time}.ok'
    if not indx1:## ok文件不存在，即当前数据未处理完成
        with open(retrynextmess,"w") as f:## 写入*.retry
            pass
        mees,status = process_ecmwf_precip(input_directory, start_time, end_time, outpath1, mess_path, example_n)  ## 开始执行任务
        indx1_new = check_data(start_time,mess_path)                 ## 刷新一次检查，indx1_new
        if indx1_new and (not os.path.exists(oknextmess)):           ## 执行完成后，rain_240已经生成，但是不存在ok文件
            with open(oknextmess,"w") as f:                          ## 写入*.ok
                pass
    else:
        if os.path.exists(retrynextmess):                            ## 检查完，rain_240存在，但是也存在retry文件
            os.remove(retrynextmess)                                 ## 删除retry
            if not os.path.exists(oknextmess):                       ## rain_240存在，retry不存在，执行生成*.ok
                with open(oknextmess,"w") as f:                      ## 写入*.ok
                    pass
    ...
    #########################
    for ii in range(1,9):
        check_time = (current_time - timedelta(hours=12*ii)).strftime("%Y%m%d%H")  ## now time - 12hours
        ...
        oknextmess    = f'{mess_path}/ECMWF/{check_time[0:8]}/rain24_ECMWF_{check_time}.ok'
        if not indx2:
            with open(retrynextmess, "w") as f:
                pass
            mees, status = process_ecmwf_precip(input_directory, check_time, check_time, outpath1, mess_path, example_n)
            indx2_new = check_data(check_time,mess_path)                 ## 刷新一次检查，indx2_new
            if indx2_new and (not os.path.exists(oknextmess)):           ## 执行完成后，rain_240已经生成，但是不存在ok文件
                with open(oknextmess,"w") as f:                          ## 写入*.ok
                    pass
        else:
            if os.path.exists(retrynextmess):                            ## 检查完，rain_240存在，但是也存在retry文件
