...
from  main  import quyu_grid,airport_interp_single,airport_interp,Plev,dimian_single_parallel
import asyncio
import sys
import yunyao_met
...
if __name__ == "__main__":
 ...
 ###################
  # UTC time
  #  now_time = dt.datetime(2025,8,30,0)
    ...
    for tg_time in np.unique(todo_tg_time):
        ...
        if not os.path.exists(jobnextmess):
            ...
            if os.path.exists(jobabovemess):
                ...
                result = asyncio.run(dimian_single_parallel(plev))
                #result["status"] = True
                for sa in result:
                    print(sa[1])
                if result["status"]:
