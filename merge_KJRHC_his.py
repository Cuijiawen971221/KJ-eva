...
def count_nc_files(nc_root: str, cycle: str) -> int:
    # 需求: 检查 /process_time/ 路径下 *TEM-EAI-*.NC 数量
...
def force_ok_to_retry(mess_root: str, cycles: List[str], logger) -> Tuple[int, int]:
    changed = 0
    created = 0
    for cycle in cycles:
        retry_path, ok_path = marker_paths(mess_root, cycle)
        os.makedirs(os.path.dirname(retry_path), exist_ok=True)
        if os.path.exists(ok_path):
            os.replace(ok_path, retry_path)
            with open(retry_path, "w", encoding="utf-8") as f:
                f.write(f"retry cycle={cycle} forced_by_history_backfill\n")
            changed += 1
            logger.info("cycle=%s 强制将ok改为retry: %s -> %s", cycle, ok_path, retry_path)
        elif not os.path.exists(retry_path):
            with open(retry_path, "w", encoding="utf-8") as f:
                f.write(f"retry cycle={cycle} forced_by_history_backfill_no_ok\n")
            created += 1
            logger.info("cycle=%s 未发现ok, 已生成retry以执行强制回算: %s", cycle, retry_path)
...
def run_history(
    ...
    log_file = os.path.join(
        ...
        "KJRHC",
        "logs",
        f"merge_KJRHC_his_{start_cycle}_{end_cycle}.log",
    )
    logger = setup_logger(log_file=log_file, verbose=True)
    logger.info("历史回算启动 start=%s end=%s cycle_count=%d", start_cycle, end_cycle, len(cycles))
    logger.info("参数 txt_root=%s nc_root=%s output_root=%s mess_root=%s", txt_root, nc_root, output_root, mess_root)

    changed_ok, created_retry = force_ok_to_retry(mess_root=mess_root, cycles=cycles, logger=logger)
    logger.info(
        "历史区间强制重算: 已将 %d 个ok改为retry, 新生成 %d 个retry",
        changed_ok,
        created_retry,
    )
    ...
    logger.info("历史回算结束 success=%d fail=%d total=%d", success_count, fail_count, len(cycles))
...
if __name__ == "__main__":
    ...
    print({
        "txt_root": conf.kjrhc_txt_root,
        "nc_root": conf.kjrhc_nc_root,
        "output_root": conf.kjrhc_output_root,
        "mess_root": conf.kjrhc_mess_root,
    })
    print({
        "start_cycle": st_str_,
        "end_cycle": ed_str_,
        "split_by_cycle": True,
        "require_nc_count": conf.kjrhc_require_nc_count,
    })
