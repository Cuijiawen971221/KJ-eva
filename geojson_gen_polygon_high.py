...
def interpolate_resolution(lon_1d, lat_1d, data_2d, scale):
    """新增：分辨率放大插值函数
    ...
    new_lon = np.linspace(lon_1d.min(), lon_1d.max(), 721)
    # 纬度：原范围[lat_min, lat_max]，点数=原点数×scale（保持纬度从高到低/低到高的顺序）
    new_lat = np.linspace(lat_1d.min(), lat_1d.max(), 361)
...
def read_grib_file(file_path):##, variable_name="tp"):
   ...
   # if True:
       ...
       # print(f"  数据形状：{ds[variable_name].shape}")
        ...
        ds.close()
        return data_2d-0.00001, lon_1d-0.00001, lat_1d-0.00001, times, time_units
...
def process_sub_poly(shapely_poly, lower, upper, time_str, features, filter_reason, vertex_stats):
    ...
    for interior in shapely_poly.interiors:
        ...
        # interior_poly = Polygon([interior_coords[::-1].tolist()] + [])  Polygon([interior_coords[::-1].tolist()])
        polygon = ShapelyPolygon(tuple(tuple(row) for row in interior_coords[::-1].tolist())  )
        # print("Polygon is valid:", polygon.is_valid)
        # print("Polygon area:", polygon.area)
        # print('interior_poly ', interior_poly)
        if len(interior_coords) >= 3 and polygon.area >= MIN_POLYGON_AREA:
...
def process_line_segment(vertices, temp_value, time_str, level_idx, filter_stats):
...
def contour_to_geojson_line(contour, time_str, output_file="temp_contours_geojson"):
    """将等温线转换为GeoJSON，过滤小闭环"""
    #os.makedirs(output_dir, exist_ok=True)
    #output_file = os.path.join(output_dir, f"temp_contour_{time_str}.geojson")
    
    features = []
    filter_stats = {
        "small_closed": 0,    # 小面积闭环
        "invalid_geom": 0,    # 无效几何
        "lat_range": 0,       # 纬度范围外
        "short_segment": 0    # 点数量不足
    }
    
    # 遍历每条等温线
    for level_idx, collection in enumerate(contour.collections):
        temp_value = contour.levels[level_idx]
        
        # 遍历线段
        for path in collection.get_paths():
            vertices = path.vertices  # 坐标数组 (n, 2)
            if len(vertices) < 2:
                filter_stats["short_segment"] += 1
                continue
            
            # 处理线段（包含闭环过滤）
            feature = process_line_segment(
                vertices=vertices,
    ...
    # 保存GeoJSON（使用ujson提升速度）
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    with open(output_file, 'w', encoding='utf-8') as f:
        ujson.dump(FeatureCollection(features), f)
    
    # 打印统计信息
    total_filtered = sum(filter_stats.values())
    print(f"\n=== GeoJSON生成完成 ===")
    print(f"  文件路径：{output_file}")
    print(f"  保留线段数量：{len(features)}")
    print(f"  总过滤数量：{total_filtered}")
    print(f"  过滤原因分布：")
...
def contour_to_geojson(contour, levels, time_str, output_file="contour.geojson"):
    ...
    for i, collection in enumerate(contour.collections):
        if i + 1 >= len(levels) or not collection.get_paths():
            continue
        #print('in i, collection in enumerate(contour.collections)', collection)
        ...
        for path in collection.get_paths():
            ...
            if not polygons:
                continue
            #print('for path in collection.get_paths()', polygons)
            exterior = polygons[0]
            ...
            try:
                ...
                shapely_poly = shapely_poly.simplify(
                    SIMPLIFY_TOLERANCE,
                ...
                process_sub_poly(shapely_poly, lower, upper, time_str, features, filter_reason, vertex_stats)
...
def process_rain_line(filename, output, custom_levels):
    # 1. 读取GRIB文件（2D数据）
    start_read = time.perf_counter()
    data_2d, lon_1d, lat_1d, times, time_units = read_grib_file(filename)#, variable_name)
    print(data_2d.dtype)
    ...
    #lon_interp, lat_interp, data_interp = lon_1d, lat_1d, data_2d
    ...
    print(f"插值后经度范围：{lon_interp.min():.4f} ~ {lon_interp.max():.4f}")
    # 4. 纬度裁剪（使用插值后的高分辨率数据）
    try:
    ...
    contour_to_geojson_line(contour, time_str, output)
    end_geojson = time.perf_counter()
    print(f"GeoJSON生成耗时: {end_geojson - start_geojson:.2f} 秒")


def process_rain(filename, output, custom_levels):
...
if __name__ == "__main__":
    ...
    #main()
    process_rain("/home/user/workshop/met/met_backend/fcst/KT1279/rain/rain24/fcst2025090100024.grb", "test/output.json", [0.1, 10.0, 25.0, 50.0, 100.0, 250.0, 1000.0])
