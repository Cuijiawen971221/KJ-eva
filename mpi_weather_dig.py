...
#    now_time = dt.datetime.strptime("2025042000","%Y%m%d%H")
    ...
    if fcst in conf.model:
        ...
        for tg in tgl: # one hour only retry 4 failed project
            ...
            if abs(tgdt.total_seconds()) < 3*24*3600:
                tgstr = dt.datetime.strptime(tg.split("/")[-1],f"synoptic_{fcst}_%Y%m%d%H.retry")
  ...
  #
            ...
            jobnextmess = jobpath + f"/synoptic_{fcst}_{tg_time.strftime('%Y%m%d%H')}.ok"
            retrynextmess = jobpath + f"/synoptic_{fcst}_{tg_time.strftime('%Y%m%d%H')}.retry"
            if not os.path.exists(jobnextmess):
                if os.path.exists(jobabovemess):
                    model = mpiModel(fstc=fcst,\
                      para="2t,2d,2r,10u,10v,sp,mslp,vis,tcc,lcc,ch,rad",\
