...
def run_main(start_time, end_time, length, inpath, outpath, model_n, stfc_timelist, mess_path, hri):
    ...
    if os.path.exists(check_filen):
        ...
        if not os.path.exists(successmess):
            with open(successmess, "w") as f:
                pass
        else:
            print('sucess mess is exists!!!')
...
def run_main_process_rain(model_n,length,inpath,outpath,mess_path,timenow):
    ...
    if not os.path.exists(check_info1):
        ...
        if os.path.exists(retrynextmess):
            os.remove(retrynextmess)
        print('文件存在，跳出')
        ok_mess = f'{mess_path}/NCEP/{start_time[0:8]}/rain24_NCEP_{start_time}.ok'
        with open(ok_mess,"w") as f:
            pass
    ...
    if not os.path.exists(check_info2):
        ...
        run_main(check_time[0:8], check_time[0:8], length, inpath, outpath, model_n, stfc_timelist2,mess_path,start_time[8:10])
    else:
        if os.path.exists(retrynextmess):
            os.remove(retrynextmess)
        print('文件存在，跳出')            
        ok_mess = f'{mess_path}/NCEP/{check_time[0:8]}/rain24_NCEP_{check_time}.ok'
        with open(ok_mess,"w") as f:
            pass
